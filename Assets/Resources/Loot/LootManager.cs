...
{
    public List<LootGroup> lootGroups;
    private static GameObject prefabCacheManager;

    void Start()
    {
        // Ensure PrefabCache instance is initialized and load all prefabs initially
        if (PrefabCache.Instance == null)
        {
            prefabCacheManager = new GameObject("PrefabCacheManager");
            prefabCacheManager.AddComponent<PrefabCache>();
        }
        PrefabCache.Instance.LoadAllPrefabs(Path.Combine(Application.dataPath, "Resources/Loot", "lootgroups.json"));
        LoadLootGroupsFromJSON("lootgroups.json");
    }

    void OnDestroy()
    {
        // Destroy the PrefabCacheManager GameObject when stopping the game
        if (prefabCacheManager != null)
        {
            Destroy(prefabCacheManager);
            prefabCacheManager = null;
        }
    }
    ...
    private static void LoadLootGroupsInEditor()
    {
        // Ensure PrefabCache instance is initialized
        if (PrefabCache.Instance == null)
        {
            prefabCacheManager = new GameObject("PrefabCacheManager");
            prefabCacheManager.AddComponent<PrefabCache>();
        ...
        // Should be almost a copy/paste for the LoadLootGroupsFromJSON method: remember to switch the path to filePath. 
        string filePath = Path.Combine(Application.dataPath, "Resources/Loot", "lootgroups.json");
        if (File.Exists(filePath))
        {
            string json = File.ReadAllText(filePath); 
            ...
            foreach (var lootGroup in lootGroups) 
            {
                foreach (var lootItem in lootGroup.lootItems)
                {
                    if (!string.IsNullOrEmpty(lootItem.itemPrefabName)) // Remember to change property name if json changes!!!!
                    {
                        GameObject prefab = PrefabCache.Instance.GetPrefabBasedOnName(lootItem.itemPrefabName); // Arthur idea to move the prefab to PreFabCache to avoid loading the same prefab multiple times
                        if (prefab != null)
                        {
                            lootItem.itemPrefab = prefab; 
                        }
            ...
            // Store the loaded loot groups in a static field for editor scripts to access
            EditorLootGroups = lootGroups; 
            Debug.Log("Loot groups loaded successfully in editor.");
        }
        else
        ...
        if (File.Exists(filePath))
        {
            ...
            foreach (var lootGroup in lootGroups)
            {
                foreach (var lootItem in lootGroup.lootItems)
                {
                    if (!string.IsNullOrEmpty(lootItem.itemPrefabName))
                    {
                        GameObject prefab = PrefabCache.Instance.GetPrefabBasedOnName(lootItem.itemPrefabName);
                        if (prefab != null)
                        {
                            lootItem.itemPrefab = prefab;
                        }
